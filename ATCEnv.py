...
# =============================================================================
# BASELINE AIRCRAFT STATE GENERATOR
# =============================================================================
def build_gradual_approach_baseline(n_aircraft=MAX_AIRCRAFT, seed=42):
    """
    Generate a fixed set of initial aircraft states representing a gradual
    approach conflict scenario in the NEC airspace.

    Aircraft are initialized in converging pairs with randomized headings,
    altitudes, and positions centered near New York airspace. Used to create
    a reproducible baseline dataset (seed=42) shared across all Monte Carlo
    evaluation runs to ensure fair comparison between baseline and AI conditions.

    Parameters
    ----------
    n_aircraft : int
        Number of aircraft to generate (default: MAX_AIRCRAFT = 15).
    seed : int
    ...
    -------
    pd.DataFrame
        Aircraft state DataFrame with columns:
    ...
    for pair in range(n_aircraft // 2):
        sep_nm = np.random.uniform(12.0, 18.0)
        half   = sep_nm / 2.0
        ...
        hdg_a += np.random.normal(0, 8)
        hdg_b += np.random.normal(0, 8)

        lat_a = pair_lat + np.random.normal(0, 0.05)
        lon_a = pair_lon - (half / NM_PER_DEG_LON)
        lat_b = pair_lat + np.random.normal(0, 0.05)
        lon_b = pair_lon + (half / NM_PER_DEG_LON)
        ...
        for i, (lat, lon, hdg, dest) in enumerate([
            (lat_a, lon_a, hdg_a, dest_a),
            (lat_b, lon_b, hdg_b, dest_b)
        ]):
            data.append([
                pair * 2 + i,
                "UAL{}".format(pair * 2 + i + 100),
                np.random.choice(airports),
                dest[2],
                np.random.choice(["jet", "turboprop"], p=[0.8, 0.2]),
                0, lat, lon, alt_base, vel, hdg, dest[0], dest[1]
            ])

    # Handle odd aircraft count
    if n_aircraft % 2 == 1:
        i = n_aircraft - 1
        data.append([
